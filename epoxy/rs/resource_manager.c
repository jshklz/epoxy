...
EPOXY_function RS_ResourceManager RS_CreateResourceManager()
{
...
EPOXY_function void RS_AddToResourceManager(RS_ResourceManager* resource_manager,
                                           void* value,
                                           i32 size,
                                           u8* key)
{
...
EPOXY_function void RS_RemoveFromResourceManager(RS_ResourceManager* resource_manager,
                                                u8* key)
{
...
EPOXY_function void* RS_GetElement(RS_ResourceManager* resource_manager, u8* key)
{
    for (i32 i = 0; i<resource_manager->used;i++)
    {
        if (U_Compare(resource_manager->resources[i].key, key)) { return resource_manager->resources[i].value; }
    }
    EPOXY_Error(key, "Resource Manager: Key Not Found");
    return 0;
