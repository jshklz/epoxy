...
{
    EPOXY_Log("OS: Win32: Virtual Allocated %i\n", size);
...
{
    EPOXY_Log("OS: Win32: Freed at %p and the size freed was %i\n", address, size);
...
OS_ConstructApp(const u8* name, V2F size, V2F pos, OS_AppStyle style)
{
    ...
    window_class.lpszClassName = "EPOXY_WINDOW_CLASS";
    ...
    {
        EPOXY_Log("OS: Win32: Registering Window Class\n");
        ...
        EPOXY_Log("OS: Win32: Created Window:\n- name: %s\n- x: %i\n- y: %i\n- w: %i\n- h: %i\n",
                 name, application->pos.x, application->pos.y, application->size.w,application->size.h);
        *running = 1;
        g_application = application;
        EPOXY_Log("OS: Win32: Constructed application successfully!\n");
...
OS_MessageBox(OS_App* app, const u8* contents, const u8* titlebar, OS_MessageBoxFlags flags)
{
    ...
    EPOXY_Log("OS: Win32: MessageBox(%s, %s)\n", contents, titlebar);
    switch (flags)
    {
...
{
    EPOXY_Log("OS: Win32: Window title was changed to \"%s\"\n", title);
    app->name = title;
    SetWindowTextA(
...
{
    EPOXY_Log("OS: Win32: Bordered Fullscreen enabled\n");
...
{
    EPOXY_Log("OS: Win32: Borderless Fullscreen enabled\n");
...
OS_SaveFile(const u8* path, const u8* contents)
{
    ...
    EPOXY_Log("OS: Win32: Attempting to write file");
    ...
    {
        EPOXY_Log("OS: Win32: Finished writing to %s! (bytes %i)\n", path, byte_written);
        return 1;
    }
    return 0;
...
WinMain(HINSTANCE instance,
        ...
        int cmd_show)
{
    EPOXY_Log("OS: Win32: WinMain entry point\n");
