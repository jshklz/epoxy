...
///~ vertex buffers
EPOXY_function GFX_VertexBuffer
GFX_CreateVertexBuffer(i32 size)
{
    GFX_VertexBuffer vbo;
    EPOXY_Log("GFX: OpenGL: Created Vertex Buffer with size: %i\n", size);
...
EPOXY_function GFX_ElementBuffer 
GFX_CreateElementBuffer(i32 size, u32* indices)
{
    GFX_ElementBuffer buffer = {0};
    //EPOXY_Log("GFX: OpenGL: Created Element Buffer with size: %i\n", size);
                 ...
                 u8* fragment_shader)
{
    ...
    {
        EPOXY_Error(shader.vertex_error, "Vertex Shader: Compilation Error");
    }
    if (shader.fragment_error)
    {
        EPOXY_Error(shader.fragment_error, "Fragment Shader: Compilation Error ");
    }
    if (shader.program_error)
    {
        EPOXY_Error(shader.program_error, "Program: Linking Error ");
    }
    
    glDeleteShader(vs);
    glDeleteShader(ps);
    EPOXY_Log("GFX: OpenGL: Compiled Vertex and Fragment shaders!\n");
    return shader;
}
EPOXY_function void
...
EPOXY_function GFX_VertexArray
GFX_CreateVertexArray(void)
{
    ...
    glBindVertexArray(0);
    EPOXY_Log("GFX: OpenGL: Created vertex array!\n");
    return vao;
...
{
    EPOXY_Log("GFX: OpenGL: Applied vertex array format!\n");
    array->flags = flags;
...
GFX_CreateRenderer(OS_App* app)
{
    ...
    EPOXY_Log("GFX: OpenGL: Enabling blending!\n");
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    
    glEnable(GL_BLEND);
    
    EPOXY_Log("GFX: OpenGL: Loaded OpenGL Renderer\n");
    EPOXY_Log("GFX: OpenGL: Version: %s\n", GFX_GetBackendVersion());
    EPOXY_Log("GFX: OpenGL: Vendor: %s\n", GFX_GetGPUVendor());
    EPOXY_Log("GFX: OpenGL: GPU name: %s\n", GFX_GetGPUName());
...
GFX_LoadDataTexture2D(u8* data, V2I size, GFX_ColorFormat format)
{
    ...
    EPOXY_Log("GFX: OpenGL: Loaded data for texture\n");
    return texture;
}
...
GFX_LoadTexture2D(u8* path)
{
    GFX_Texture2D texture = {0};
    texture.path = path;
    EPOXY_Log("GFX: OpenGL: Attempting to load \"%s\"\n", path);
    u8* data = stbi_load(path, &texture.size.w, &texture.size.h, &texture.channels, 0);
    
    if (data == 0)
    {
        EPOXY_Log("GFX: OpenGL: \"%s\" was invalid! Can't load image, returing texture as it is.\n", path);
...
GFX_CreateFramebuffer(GFX_Renderer* renderer, GFX_ColorFormat color_format)
{
...
#if 0 
    EPOXY_Log("GFX: OpenGL: Creating a renderbuffer!\n");
    u32 rbo;
    ...
    EPOXY_Log("GFX: OpenGL: Done creating a renderbuffer!\n");
#endif
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        EPOXY_Log("GFX: OpenGL: Framebuffer was not complete.\n");
    EPOXY_Log("GFX: OpenGL: Framebuffer Status: %i\n",glCheckFramebufferStatus(GL_FRAMEBUFFER));
    
    GFX_PopFramebuffer();
    
    EPOXY_Log("GFX: OpenGL: Created framebuffer!\n");
