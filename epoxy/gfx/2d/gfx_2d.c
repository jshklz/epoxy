...
GFX_CreateRect(V2F size, V2F pos)
{
    ...
    EPOXY_Log("GFX2D: Rectangle: Creating generic primitive: Rectangle.\n");
...
// text
EPOXY_function GFX_Font
GFX_CreateFont(const u8* font_path, i32 size)
{
    GFX_Font font = {0};
    
    EPOXY_Log("GFX2D: Font: Creating the font (%s)\n", font_path);
    ...
    EPOXY_Log("GFX2D: Font: Packing the font...\n");
    
    stbtt_pack_context context;
    if (!stbtt_PackBegin(&context, font.atlas_data, 1024, 1024, 0, 1, 0))
    {
        EPOXY_Log("GFX2D: Font: Failed to load font.\n");
        return font;
    }
    ...
    {
        EPOXY_Log("GFX2D: Font: Failed to pack font.\n");
    }
    
    stbtt_PackEnd(&context);
    EPOXY_Log("GFX2D: Font: Packed the font and finished!\n");
    EPOXY_Log("GFX2D: Font: Creating the texture!\n");
...
EPOXY_function GFX_Text
GFX_CreateText(u8* text,  V2F pos, GFX_Font font)
{
    GFX_Text gfx_text = {0};
    EPOXY_Log("GFX2D: Text: Creating text with the message: %s\n", text );
    gfx_text.text = text;
    gfx_text.font = font;
    gfx_text.pos  = pos;
