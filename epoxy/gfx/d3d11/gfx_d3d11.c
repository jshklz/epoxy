...
#pragma message ( "EPOXY: d3d11: This pipeline is extremely experimental, and may not have the correct standard behaviour. Also, this pipeline will not work." )
...
#if 0 
    texture.path = path;
    EPOXY_Log("GFX: D3D11: Attempting to load \"%s\"\n", path);
    u8* data = stbi_load(path, &texture.size.w, &texture.size.h, &texture.channels, 0);
    
    if (data == 0)
    {
        EPOXY_Log("GFX: D3D11: \"%s\" \was invalid! Can't load image, returing texture as it is.\n", path);
...
GFX_CreateRenderer(OS_App* app)
{
    ...
    {
        EPOXY_Error(renderer->default_shader.vertex_error, "Vertex Shader: Compilation Error (internal-default-shader)");
    }
    if (renderer->default_shader.fragment_error)
    {
        EPOXY_Error(renderer->default_shader.fragment_error, "Fragment Shader: Compilation Error (internal-default-shader)");
    }
    if (renderer->default_shader.program_error)
    {
        EPOXY_Error(renderer->default_shader.program_error, "Program: Error (internal-default-shader)");
    }
    ...
    EPOXY_Log("GFX: D3D11: Loaded D3D11 Renderer\n");
    EPOXY_Log("GFX: D3D11: Version: %s\n", GFX_GetBackendVersion());
    EPOXY_Log("GFX: D3D11: Vendor: %s\n", GFX_GetGPUVendor());
    EPOXY_Log("GFX: D3D11: GPU name: %s\n", GFX_GetGPUName());
