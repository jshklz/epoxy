...
SND_AudioCallback(ma_device* device, void* output, const void* input, ma_uint32 frame_count)
{
    ...
    if (decoder == 0)
        EPOXY_Log("SND: Miniaudio: Failed to decode!\n");
...
SND_CreateSound(u8* path)
{
    ...
    {
        EPOXY_Log("SND: Miniaudio: Failed to decode file.\n");
        return 0;
    }
    ...
    {
        EPOXY_Log("SND: Miniaudio: Failed to open playback device.\n");
    ...
    EPOXY_Log("SND: Miniaudio: Created sound object \"%s\"\n", path);
    return sound;
...
SND_ResumeSound(SND_Sound* sound)
{
    ...
    {
        EPOXY_Log("SND: Miniaudio: Can't start playback device [[\"%s\"]]\n", sound->path);
        ma_device_uninit(&sound->device);
        ma_decoder_uninit(&sound->decoder);
    }
    
    EPOXY_Log("SND: Miniaudio: Playing audio.\n");
...
SND_SetSoundTime(SND_Sound* sound, i32 position)
{
    ...
    {
        EPOXY_Log("SND: Miniaudio: Could not change the time position.\n");
    }
}
...
SND_GetElapsedSoundTime(SND_Sound* sound)
{
    ...
    if (ma_decoder_get_cursor_in_pcm_frames(&sound->decoder,
                                            &time) != MA_SUCCESS)
    {
        EPOXY_Log("SND: Miniaudio: Could not get the time elapsed.\n");
...
SND_GetSoundVolume(SND_Sound* sound)
{
    ...
    {
        EPOXY_Log("SND: Miniaudio: Failed to get master volume.\n");
    }
    return volume*100;
}
...
SND_SetSoundVolume(SND_Sound* sound, r32 volume)
{
    ...
    {
        EPOXY_Log("SND: Miniaudio: Could not set audio volume.\n");
    }
    
    EPOXY_Log("SND: Miniaudio: Set sound volume to %f.\n",volume);
}
...
SND_PauseSound(SND_Sound* sound)
{
    ...
    {
        EPOXY_Log("SND: Miniaudio: Failed to stop audio.\n");
        ma_device_uninit(&sound->device);
        ma_decoder_uninit(&sound->decoder);
    }
    EPOXY_Log("SND: Miniaudio: Stopped audio.\n");
}


EPOXY_function SND_SoundFlag
