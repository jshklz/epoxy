...
MEM_ArenaAlloc(MEM_Arena* arena, i32 size)
{
    ...
    if (arena->end + size <= arena->size){
        ...
        arena->end += size;
        EPOXY_Log("MEMORY: Allocated %i bytes at location %p\nMEMORY: overall arena usage before resize: (%i\\%i)\n", size, loc,
                 arena->end,
                 arena->size);
        return loc;
    } else 
    {
        EPOXY_Log("MEMORY: Attempt to allocate %i bytes was refused.\n", size);
        EPOXY_Log("MEMORY: Out of memory (%i bytes used) Resizing to accomodate this change!\n", arena->size);
...
MEM_ArenaReAlloc(MEM_Arena* arena, i32 size)
{
    ...
    EPOXY_Log("MEMORY: Resized to %i bytes\n", arena->size);
...
MEM_ArenaFree(MEM_Arena* arena)
{
    ...
    EPOXY_Log("MEMORY: Arena was freed.\n");
}
...
MEM_Init(void)
{
    ...
    EPOXY_Log("MEMORY: Arena was initialized.\n");
